...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    Serial.println("已接受指令");
    switch (command) {
      case '1': // 電動缸正轉
        Serial.println("電動缸正轉1次");
        moveActuator(ELECTRIC_FRONT_PIN);
        Serial.println("DONE");
        break;
      case '2': // 電動缸反轉
        Serial.println("電動缸反轉1次");
        moveActuator(ELECTRIC_BACKWARD_PIN);
        Serial.println("DONE");
        break;
      case '3': // 油門馬達正轉
        Serial.println("油門馬達正轉1次");
        pulseOutput(GAS_PUL_PIN,1,50); // 正轉方向
        Serial.println("DONE");
        break;
      case '4': // 油門馬達反轉
        Serial.println("油門馬達反轉1次");
        pulseOutput(GAS_PUL_PIN,0,50); // 反轉方向
        Serial.println("DONE");
        break;
      case '5': // 煞車馬達正轉
        Serial.println("煞車馬達正轉1次");
        pulseOutput(BREAK_PUL_PIN,1,500); // 正轉方向
        Serial.println("DONE");
        break;
      case '6': // 煞車馬達反轉
        Serial.println("煞車馬達反轉1次");
        pulseOutput(BREAK_PUL_PIN,0,500); // 反轉方向
        Serial.println("DONE");
        break;
      default:
        // 其他指令不執行
...
{
  if(dir==1 && pulPin==GAS_DIR_PIN ){
    digitalWrite(GAS_DIR_PIN, HIGH); 
  } 
  else if(dir==1 && pulPin==BREAK_DIR_PIN ){
    digitalWrite(BREAK_DIR_PIN, HIGH);
  }
  else if(dir==0 && pulPin==GAS_DIR_PIN) {
    digitalWrite(GAS_DIR_PIN, LOW);
  }
  else if(dir==0 && pulPin==BREAK_DIR_PIN) {
