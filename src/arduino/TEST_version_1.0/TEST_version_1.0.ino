...
unsigned long pressStartTime = 0;  //計時用
...
void setup() {
  ...
  digitalWrite(FRONT_PIN, HIGH); //電缸前進
  digitalWrite(BACKWARD_PIN, HIGH); //電缸後退
  ...
  while (true) {
    if (Serial.available()) {
      ...
      switch (input) {
        case 'W': // 油門按下一定量
          Serial.println("油門加速");
          pulseOutput(GAS_PUL_PIN, 1, 20);
          break;
        case 'S': // 油門放鬆一定量
          Serial.println("油門放鬆");
          pulseOutput(GAS_PUL_PIN, 2, 20);
          break;
        case 'D': // 電缸前進
          Serial.println("電缸前進");
        ...
        case ' ': // 空白鍵：煞車（油門鬆到底，煞車按下）
          ...
          pulseOutput(BREAK_PUL_PIN, 1, 1000); // 煞車啟動
          delay(3000); // 假設車停了
          pulseOutput(BREAK_PUL_PIN, 2, 1000); // 鬆開煞車
          Serial.println("煞車完成");
          break;
        case 'P': // 結束手動控制
